...
def tf_m_q_relu(x,name=None):
    name = 'm_q_relu' + str(np.random.randint(0, 1E+2))
    with tf.name_scope(name=name) as name:
...
def tf_d_m_q_relu(x,name=None):
    name = 'd_m_q_relu' + str(np.random.randint(0, 1E+2))
    with tf.name_scope(name, "d_m_q_relu", [x]) as name:
        y = tf.py_function(np_d_m_q_relu_32,
                        [x],
                        [tf.float32],
                        name=name)
        return y[0]
