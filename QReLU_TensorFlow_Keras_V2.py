...
# Generating a unique name to avoid duplicates:
    ...
    with g.gradient_override_map({"PyFunc": rnd_name}):
        x = tf.py_function(func, inp, Tout, name=name)
        return x

np_q_relu_32 = lambda x: np_q_relu(x).astype(np.float32)

def tf_q_relu(x,name=None):
    rnd_name = 'q_relu' + str(np.random.randint(0, 1E+2))
    with tf.name_scope(name=rnd_name) as name:
...
def tf_d_q_relu(x,name=None):
    rnd_name = 'd_q_relu' + str(np.random.randint(0, 1E+2))
    with tf.name_scope(name=rnd_name) as name:
        y = tf.py_function(np_d_q_relu_32,
                        [x],
                        [tf.float32],
                        name=name,
                        stateful=False)
